...
def build_fuzzy_system():
    ...
    kebutuhan   = ctrl.Consequent(np.arange(0, 101, 1), 'kebutuhan', defuzzify_method='centroid')
    ...
    rules = [
        ctrl.Rule(suhu['tinggi'] & kel_tanah['kering'] & curah_hujan['rendah'], kebutuhan['banyak']),
        ...
        ctrl.Rule(suhu['sedang'] & kel_udara['sedang'] & curah_hujan['sedang'],  kebutuhan['sedang']),
...
# spk fuzzy page
    ...
    with col2:
        ...
        growth_stage = st.selectbox("Tahap Pertumbuhan", sorted(df['Crop_Growth_Stage'].unique()))
    ...
    with tab2:
        st.pyplot(plot_membership(fuzzy['kel_tanah'], "Kelembaban Tanah", soil_moisture))
    with tab3:
        st.pyplot(plot_membership(fuzzy['curah_hujan'], "Curah Hujan (norm.)", rainfall_norm))
    with tab4:
    ...
    rules_df = pd.DataFrame([
        ["R1",  "Suhu TINGGI", "Kel.Tanah KERING", "Hujan RENDAH", "-",               "Kebutuhan BANYAK"],
        ...
        ["R8",  "Suhu SEDANG", "-",                "Hujan SEDANG", "Kel.Udara SEDANG","Kebutuhan SEDANG"],
    ...
    if st.button("Hitung Kebutuhan Air", type="primary"):
        ...
        try:
            ...
            with col_r2:
                ...
                deg_sedikit = round(float(fuzz.interp_membership(universe, mf_sedikit, hasil)), 4)
                deg_sedang  = round(float(fuzz.interp_membership(universe, mf_sedang,  hasil)), 4)
                deg_banyak  = round(float(fuzz.interp_membership(universe, mf_banyak,  hasil)), 4)
                ...
                st.dataframe(defuzz_df, use_container_width=True, hide_index=True)
            ...
            ranking_df = pd.DataFrame({
                "Alternatif" : ["Irigasi Banyak", "Irigasi Sedang", "Irigasi Sedikit"],
                "Nilai Fuzzy": [
            ...
            ringkasan = pd.DataFrame({
                "Parameter": [
                    "Suhu Udara", "Kelembaban Tanah",
                    "Curah Hujan (aktual)", "Curah Hujan (norm.)",
                    "Kelembaban Udara", "Jenis Tanaman",
                    "Jenis Tanah", "Tahap Pertumbuhan"
                ],
                "Nilai": [
                    f"{temperature} C", f"{soil_moisture}%",
                    f"{rainfall_raw} mm/hari", f"{rainfall_norm}/100",
                    f"{humidity}%", crop_type,
            ...
            st.info("Coba ubah nilai input — beberapa kombinasi mungkin tidak tercakup oleh rule manapun.")
...
# page visualisasi
    ...
    avg_temp = df.groupby('Crop_Type')['Temperature_C'].mean().sort_values(ascending=False)
...
# page profil kelompok
    ...
    with col1:
        ...
        st.write("Sistem Pendukung Keputusan (SCPK)")
    ...
    with col2:
        ...
        | Atribut | Detail |
        |---------|--------|
        | Judul | SPK Kebutuhan Air Irigasi |
